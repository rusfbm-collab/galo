...
export const weightFreeZh: TranslationDictionary = {
  ...
  "Neural AI learns by changing hidden numerical weights. GALO has no learned weights and uses no backpropagation. It learns by building, checking, reusing and locally revising explicit objects, relations, competing explanations and evidence in a versioned Atlas, so every learned change can be read, versioned and replayed. The first product is intended to apply that architecture to decision assurance around the digital twins and operational systems industry already runs. None of it is running at an industrial site today.":
    "神经网络人工智能靠改变隐藏的数值权重来学习。GALO 没有已学权重，也不使用反向传播。它的学习方式是另一种：在带版本的 Atlas 中构建、检查、复用并就地修订显式的对象、关系、相互竞争的解释和证据，因此每一次学到的改动都可以被读取、被版本化、被重放。第一款产品计划把这套架构用于决策保障，围绕工业界已经在运行的数字孪生与运营系统。目前它还没有在任何一个工业现场运行。",
...
export const weightFreeAr: TranslationDictionary = {
  ...
  "Neural AI learns by changing hidden numerical weights. GALO has no learned weights and uses no backpropagation. It learns by building, checking, reusing and locally revising explicit objects, relations, competing explanations and evidence in a versioned Atlas, so every learned change can be read, versioned and replayed. The first product is intended to apply that architecture to decision assurance around the digital twins and operational systems industry already runs. None of it is running at an industrial site today.":
    "يتعلَّم الذكاءُ الاصطناعي العصبيّ بتغيير أوزانٍ عددية خفيّة. أمّا GALO فلا أوزان متعلَّمة لديه ولا يستعمل الانتشار العكسيّ. إنّه يتعلَّم ببناء أشياء وعلاقاتٍ وتفسيراتٍ متنافسة وأدلّةٍ صريحة داخل Atlas ذي إصدارات، وبفحصها وإعادة استعمالها ومراجعتها موضعيًّا، فيصير كلُّ تغيُّرٍ متعلَّم قابلًا للقراءة والتأريخ وإعادة التشغيل. ويُزمَع أن تُطبَّق هذه البنية في المنتج الأوّل على ضمان القرار حول التوائم الرقمية والأنظمة التشغيلية التي تعمل في الصناعة بالفعل. ولا يعمل شيءٌ من ذلك في أيّ موقعٍ صناعيّ اليوم.",
